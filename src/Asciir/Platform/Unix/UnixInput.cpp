...
namespace Asciir
{
	...
	void Input::update(TRUpdateInfo info)
	{
		...
		UnixARAttr& h_attr = dynamic_cast<UnixARAttr&>(AREngine::getEngine()->getTerminal()->getRenderer()->getAttrHandler());
	...
	{
		UnixARAttr& h_attr = dynamic_cast<UnixARAttr&>(AREngine::getEngine()->getTerminal()->getRenderer()->getAttrHandler());
		Display* x_display = h_attr.m_display;
		Window x_win = h_attr.m_window;
		Window r_win, c_win;
		int r_x, r_y, w_x, w_y;
